...
/** /build: are you still on your build, and what's the ladder of gyms it
 *  unlocks. Two questions, one screen — both come from the same ratio rules. */
export function Build({ gyms, player, modifiers, gate, stage, energyPerDay }: RouteProps) {
  ...
  // "Tus datos" vive en el shell, arriba de la ruta: cargar la API key cambia
  // de jugador sin desmontar Build, asi que un useState sembrado una sola vez
  // se quedaba con el stat primario del jugador de ejemplo. Re-sembrar durante
  // el render (el patron documentado de React para estado derivado de props)
  // evita el efecto y el frame con el valor viejo.
  const [seededFrom, setSeededFrom] = useState(defaultPrimary);
  if (seededFrom !== defaultPrimary) {
    setSeededFrom(defaultPrimary);
  ...
  }, [roadmap, gyms, stats, modifiers, player, gate, energyPerDay]);
  ...
  return (
    <>
      <BuildRatio gyms={gyms} player={player} gate={gate} hasBuild={stage.hasBuild} />
      ...
      <section className="panel">
        ...
        <p className="roadmap-desc">
          ...
          leave <strong>{roadmap.dumped.map((d) => STAT_LABEL[d]).join(' & ')}</strong> behind to
          keep the ratio. {STAT_LABEL[primary]} is currently{' '}
              ...
              <div className="stage-main">
                ...
                {s.status === 'next' && s.trainStat != null && s.targetValue != null && (
                  <div className="stage-action">
                    ...
                    {fmtInt(s.gap ?? 0)}) to unlock — a {baselineDots.toFixed(1)}→
                    {s.dots.toFixed(1)} dot upgrade (+
                    {(((s.dots - baselineDots) / baselineDots) * 100).toFixed(1)}% per train).
                    {eta && (
                      <>
                        {' '}
        ...
        <p className="footnote">
          ...
          second-highest; the paired gym (7.5) needs its pair ≥1.25× the other pair. ETAs use your
          max happy and current best gym, so they're approximate — and slightly conservative, since
          the dot upgrades speed you up along the way.
        </p>
      </section>
    </>
  );
